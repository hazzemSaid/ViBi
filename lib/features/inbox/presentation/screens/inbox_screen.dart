...
import 'package:vibi/features/inbox/presentation/widgets/inbox_widgets.dart';
...
class _InboxScreenState extends State<InboxScreen> {
  ...
  Future<void> _deleteQuestion(BuildContext context, String questionId) async {
    ...
    if (!state.hasError) {
      ...
      messenger.showSnackBar(
        const SnackBar(content: Text('Question deleted'), duration: Duration(seconds: 2)),
      );
    } else {
      setState(() {
  ...
  Future<void> _archiveQuestion(BuildContext context, String questionId) async {
    if (_hiddenQuestionIds.contains(questionId) || _archivingQuestionIds.contains(questionId)) return;
    ...
    if (state is ArchiveQuestionSuccess) {
      ...
      context.read<PendingQuestionsCubit>().updateQuestionStatus(questionId, 'archived');
      if (!mounted) return;
      messenger.showSnackBar(
        const SnackBar(content: Text('Question archived'), duration: Duration(seconds: 2)),
      );
      return;
    }

    setState(() => _hiddenQuestionIds.remove(questionId));
    final message = state is ArchiveQuestionFailure ? state.message : 'Failed to archive question';
   ...
   */
  Future<void> _unarchiveQuestion(BuildContext context, String questionId) async {
    ...
    if (state is ArchiveQuestionSuccess) {
      ...
      if (!mounted) return;
      messenger.showSnackBar(
        const SnackBar(content: Text('Moved back to pending'), duration: Duration(seconds: 2)),
      );
      return;
    }

    final message = state is ArchiveQuestionFailure ? state.message : 'Failed to move question to pending';
    messenger.showSnackBar(SnackBar(content: Text(message), backgroundColor: Colors.red));
  }
        ...
        body: SafeArea(
          child: BlocBuilder<PendingQuestionsCubit, PendingQuestionsState>(
            builder: (context, questionsState) {
              if (questionsState is PendingQuestionsLoading || questionsState is PendingQuestionsInitial) {
              ...
              if (questionsState is PendingQuestionsError) {
                return Center(
                  child: Column(
                    ...
                    children: [
                      Icon(Icons.error_outline, size: 48, color: theme.colorScheme.onSurface.withValues(alpha: 0.3)),
                      SizedBox(height: AppSizes.r16),
                      Text('Failed to load questions', style: theme.textTheme.bodyLarge?.copyWith(color: theme.colorScheme.onSurface)),
                      ...
                      SizedBox(height: AppSizes.r16),
                      ElevatedButton(
                        onPressed: () => context.read<PendingQuestionsCubit>().refresh(),
              ...
              if (questionsState is! PendingQuestionsSuccess) return const SizedBox.shrink();

              final filteredQuestions = filterQuestions(questionsState.questions, _selectedFilter)
                  .where((q) => !_hiddenQuestionIds.contains(q.id))
                  .toList();
              ...
              return NotificationListener<ScrollNotification>(
                ...
                child: RefreshIndicator(
                  ...
                  child: ListView.builder(
                    padding: EdgeInsets.fromLTRB(AppSizes.r16, AppSizes.s8, AppSizes.r16, AppSizes.r24),
                    itemCount: listStart +
                        (filteredQuestions.isEmpty ? 1 : filteredQuestions.length) +
                        (questionsState.isLoadingMore ? 1 : 0) +
                        (!questionsState.hasMore && filteredQuestions.isNotEmpty ? 1 : 0),
                    itemBuilder: (context, index) {
                      if (index == 0) {
                        return InboxTopBar(
                          totalQuestions: questionsState.questions
                              .map((q) => q.status)
                              .where((status) => isNotAnsweredStatus(status) && !isArchivedStatus(status))
                              .length,
                      ...
                      final questionEnd = listStart + (filteredQuestions.isEmpty ? 1 : filteredQuestions.length);
                      ...
                      if (index < questionEnd) {
                        ...
                        return Padding(
                          ...
                          child: QuestionCard(
                            ...
                            onDelete: () => _deleteQuestion(context, question.id),
                            onArchive: () => _archiveQuestion(context, question.id),
                            onUnarchive: () => _unarchiveQuestion(context, question.id),
                      ...
                      if (questionsState.isLoadingMore && index == questionEnd) {
                        return Padding(
                      ...
                      return Padding(
                        ...
                        child: Center(
                          child: Column(
                            children: [
                              Icon(Icons.check_circle_outline, size: 44, color: theme.colorScheme.onSurface.withValues(alpha: 0.3)),
                              SizedBox(height: AppSizes.r12),
                              Text(
                                'No more questions',
