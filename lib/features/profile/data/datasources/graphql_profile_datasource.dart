...
class GraphQLProfileDataSource implements ProfileDataSource {
  ...
  @override
  Future<Either<String, void>> updateProfile(UserProfileModel profile) async {
    ...
    if (records == null || (records as List).isEmpty) {
      return left('Profile update failed: No rows affected');
    }
    return right(null);
  }
  ...
  Future<Either<String, String>> uploadProfileImage(
    ...
    try {
      ...
      return right(_client.storage.from('avatars').getPublicUrl(filePath));
    } catch (error) {
      return left(SupabaseErrorHandler.getErrorMessage(error));
    }
  }
  ...
  Future<Either<String, List<String>>> uploadAndSaveAvatar(
    ...
    final uploadedUrlResult = await uploadProfileImage(uid, image);

    return uploadedUrlResult.fold((error) => left(error), (uploadedUrl) async {
  ...
  Future<Either<String, String>> uploadPublicProfileImage(
    ...
    if (slot < 1 || slot > 3) {
      return left('slot must be between 1 and 3');
    }
  ...
  Future<Either<String, PublicProfileModel>> getPublicProfile(
    ...
    final edges = result.data?['profilesCollection']?['edges'] as List? ?? [];
    if (edges.isEmpty) return left("Profile not found for user ID: $userId");

    final node = edges.first['node'] as Map<String, dynamic>;
    final profile = await _buildPublicProfileFromNode(
      node: node,
  ...
  Future<Either<String, PublicProfileModel>> getPublicProfileByUsername(
    ...
    final edges = result.data?['profilesCollection']?['edges'] as List? ?? [];
    if (edges.isEmpty) return left("Profile not found for username: $username");
